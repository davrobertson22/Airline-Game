...
// (src/utils/simulation.js — `totalRevenue += result.revenue`
...
//
// ── STATUS: PARTIALLY RED, AND DELIBERATELY NOT WIRED INTO `npm test` ────────
//
// The Away Digest half of this suite is GREEN — src/utils/awayDigest.js was
// carrying the same double count and has been fixed.
//
// The Statistics half is still RED. Its single cause is one line in
// src/store/GameContext.jsx (the ADVANCE_WEEK statsEntry, ~line 3398):
//
//     revenue: (report.totalRevenue ?? 0) + (report.totalCargoRevenue ?? 0),
//
// which must become:
//
//     revenue: report.totalRevenue ?? 0,
//
// (Headwinds carries the identical record in packages/engine/src/reducer.mjs and
// has already been patched exactly that way; its golden master still prints
// PARITY OK, because cargo is a breakdown line and the seed scenario flies none.)
//
// GameContext.jsx was off-limits to the worker who wrote this suite, so the fix
// was left undone rather than faked. Apply the one-liner above, then add
//
//     && node --import ./tools/_register-loader.mjs tools/statistics-cargo-test.mjs
//
// to the `test` script in package.json. Until then this file must stay out of
// the test script — it exits non-zero on purpose.
// ─────────────────────────────────────────────────────────────────────────────
...
import { cargoReferenceYield, referencePrice } from '../src/utils/market.js';
import { defaultClassPrices, formatMoney } from '../src/utils/simulation.js';
