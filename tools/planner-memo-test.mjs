...
const { AIRPORTS } = await import('../src/data/airports.js');
const { AIRCRAFT_TYPES } = await import('../src/data/aircraft.js');
const { rankAircraftForRoute, seasonalProfitByType } = await import('../src/models/aircraftRecommender.js');
const { projectRouteAddition } = await import('../src/models/pairShare.js');
const { routeDistanceKm, effectiveRangeKm, currentGameDate } = await import('../src/utils/simulation.js');
...
test('the seasonal strip is the same warm and cold', () => {
  const warm = seasonalProfitByType(s, { ...spec, types: types.slice(0, 3) });
  const cold = seasonalProfitByType({ ...s }, { ...spec, types: types.slice(0, 3) });
  assert.deepEqual([...warm.byType.entries()].map(([k, v]) => [k, v.byMonth]),
                   [...cold.byType.entries()].map(([k, v]) => [k, v.byMonth]));
});
