...
//   H10 `directionalLoadMultiplier` and `loadDemandScale` both apply
...
//       many times oversubscribed in a skewed month was docked a haircut it
//       could not lose, and the LOAD_CEILING (0.95) was unreachable: a deeply
//       oversubscribed route landed at the same load factor as one at parity.
...
//       revenue.
//
// Tailwinds notes vs the Headwinds twin this is ported from:
//   • The load-factor ceiling here is UNCONDITIONAL (utils/market.js
//     LOAD_CEILING = 0.95 + a per-pair-per-week `route.loadJitter` the tick
//     attaches). There is no new-world-restrictions switch to turn it on, so
//     the ceiling tests drive the DEFAULT path (jitter absent = expected week).
//   • The reducer lives in src/store/GameContext.jsx, so this suite needs the
//     JSX loader.
//
//   node --import ./tools/_register-loader.mjs tools/demand-conservation-test.mjs
...
test('H9: the contested fold agrees with the monopoly fold', () => {
  const market = buildRouteMarket('JFK', 'LAX', GD);
  // Above reference, so both shapes are DEMAND-bound. At the reference fare
  // this pair's pool exceeds 42 narrowbodies' worth of seats and both sides cap
  // at the seat count (7,560 = 7,560 on HEAD), which hides the deletion behind
  // the cap — the same trap the test above sidesteps.
  const fare = Math.round(market.referencePrice * 1.5);
...
// stay full) and the correct landing point is the structural ceiling.
...
test('H10: a route drowning in demand is not docked for seasonal skew', () => {
  ...
  // With demand many times capacity, BOTH directions saturate: the directional
  // multiplier is exactly 1.0 and the only haircut left is the structural
  // spill ceiling (0.95). On HEAD the skew haircut stacked on top of it.
  assert.ok(res.loadFactor > 0.94,
...
test('H10: the structural load ceiling is reachable when demand is deep', () => {
  const { route, aircraft } = saturatedRoute(FAT[0], FAT[1]);
  const res = simulateRoute(route, aircraft, GD, null, 1.0, null, [], null, null, 1.0, null, []);
  assert.ok(res, 'fixture did not simulate');
  // Deep oversubscription asymptotes to LOAD_CEILING (0.95) x jitter (absent = 1).
  // On HEAD it landed at ~0.87 — indistinguishable from a route at parity.
  assert.ok(res.loadFactor > 0.93,
    `a many-times-oversubscribed route landed at ${(res.loadFactor * 100).toFixed(1)}%, ceiling unreachable`);
});

test('H10: a route at rough parity still lands well below the ceiling', () => {
...
function buildAirline(shape, fare) {
  ...
  s = { ...s, competitors: [], encroachments: {}, cash: 400_000_000 };
