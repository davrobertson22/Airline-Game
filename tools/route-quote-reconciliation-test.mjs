...
//
// Tailwinds port of the Headwinds suite of the same name. The world here is the
// one the app really runs: built through the reducer, then passed through
// reconcileState exactly as GameProvider does on load, so the AI carriers
// START_GAME seeds are in every quote AND in the tick. The multiplayer-only
// scenarios (human rival, restricted world) become an AI rival on the pair.
//
...
//   · a pair an AI carrier contests            (rival channel)
...
import { defaultClassPrices } from '../src/utils/simulation.js';
import { referencePrice, LOAD_JITTER } from '../src/utils/market.js';
import { findCandidates, scoreCandidates } from '../src/models/routeFinder.js';
 ...
 */
function world({ seed = 4242, year = 1, week = 20, incumbent = null } = {}) {
  seedRandom(seed);
  let s = gameReducer(freshState(),
    { type: 'START_GAME', airlineName: 'Reconcile Air', hub: HUB, enableObjectives: false });
  // Classic labour: START_GAME turns the crew pipeline on with a headcount of
  // zero, and a seven-tail fixture that never hires would park its fleet.
  s = { ...s, cash: 5e9, crewPipeline: false };
  ...
  // The state the app actually runs: what GameProvider makes of this save.
  const state = reconcileState(JSON.parse(JSON.stringify({
  ...
  return { state, spare: state.fleet.find(a => a.id === spare.id) };
}
...
// GameProvider IS the production path in Tailwinds: it loads the save and runs
// reconcileState on it — the same pass world() already applied, so the planner
// renders over the state the tick runs on.
const { GameProvider, gameReducer, freshState, reconcileState } = await import('../src/store/GameContext.jsx');
...
function reconcile(name, state, spec) {
  ...
  // Tailwinds' tick attaches a deterministic ±LOAD_JITTER load wobble to every
  // route, keyed on pair + absolute week (weeklyLoadJitter); the previews quote
  // the EXPECTED week (jitter = 1) on purpose (pairShare.js, the note above
  // runAt). Demand-side agreement is therefore "within the wobble"; the cost
  // side and the connecting recipe stay exact. (Headwinds only wobbles in NWR
  // worlds, so its copy of this suite holds classic worlds to 1%.)
  const demandPct = ENGINE_PCT + LOAD_JITTER;
  ...
  push('finder',  'connecting', f.connecting, t.connecting, 100, demandPct, rev);
  ...
  push('planner', 'connecting', p.connecting, t.connecting, p.tol.connecting + 100, demandPct, rev);
...
// 4. An AI carrier on the pair, in the shape START_GAME seeds and the AI tick
//    maintains (a route config on the competitor; undercutting the reference).
test('pair contested by an AI carrier: three quotes agree', () => {
  const { state: base, spare } = world();
  const freq = uncappedFreq(base, { origin: HUB, dest: PROBE, frame: spare });
  const key  = [HUB, PROBE].sort().join('-');
  const [rival, ...rest] = base.competitors;
  assert.ok(rival, 'fixture: START_GAME seeded no AI carriers');
  const state = {
    ...base,
    competitors: [{ ...rival, routes: { ...(rival.routes ?? {}), [key]: { frequency: 21, priceMultiplier: 0.95 } } }, ...rest],
  };
  const r    = reconcile('AI rival', state, { origin: HUB, dest: PROBE, frame: spare, freq });
...
// 5. Year four. Demand has compounded for three years; a preview that builds its
...
// 6. Point-to-point off the hub: no feed at either end, the simplest case.
...
// 7. Lease treatment. The tick charges the tail's SIGNED rate, not the catalogue
