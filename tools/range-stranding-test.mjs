...
//
// THE FIX (applyRangeStranding, store/GameContext.jsx): flag the route once,
// write a durable news row (plus a toast mid-game), badge it on the Routes and
// cargo lists, and clear the flag the moment it is flyable again. Nothing is
// closed — REASSIGN_ROUTE already moves a route to a longer-range tail while
// keeping its ramp, which is the cheap fix the notice points at.
//
...
const { gameReducer, freshState, reconcileState, applyRangeStranding } =
...
const strandNews = (s) => (s.newsLog ?? []).filter(n => n.kind === 'route_out_of_range');
...
test('a stranded route is flagged once, with one news row and one toast', () => {
  const s = shortOf(flying());
  ...
  assert.equal(strandNews(next).length, 1, 'expected exactly one news row');
  const row = strandNews(next)[0];
  assert.equal(row.tier, 1, 'a route that stopped flying is headline news');
  assert.equal(row.data.sectorKm, r.rangeStranded.sectorKm);
...
test('the flag clears when the aircraft can reach the route again', () => {
  ...
  assert.equal(strandNews(next).length, strandNews(stranded).length, 'a repair must not post news');
});
...
test('{ toast: false } still writes the news row', () => {
  const s = shortOf(flying());
  const next = applyRangeStranding(s, { toast: false });
  assert.equal(strandNews(next).length, 1);
...
test('ADVANCE_WEEK flags the route, reports it once, and still advances exactly one week', () => {
  ...
  assert.equal(strandNews(s).length, 1);
...
test('loading a save flags stranded routes and writes the news — without a toast', () => {
  const s = shortOf(flying());
  const loaded = reconcileState(JSON.parse(JSON.stringify(s)));
  const r = (loaded.routes ?? []).find(x => x.id === s.routes[0].id);
  assert.ok(r?.rangeStranded, 'load did not flag the stranded route');
  assert.equal(strandNews(loaded).length, 1);
  assert.equal((loaded.pendingToasts ?? []).length, (s.pendingToasts ?? []).length,
    'no toast on load — the news row is the durable record');
});
