...
test('classic games reproduce the published table exactly (parity) — except lines closed long enough to ramp', () => {
  const ramped = t => (t.deliveredAgeWeeks ?? 0) > 0 && t.oop != null && 2026 - t.oop >= VINTAGE_RAMP_FROM_YEARS;
  for (const t of AIRCRAFT_TYPES) if (!ramped(t)) assert.equal(eraDeliveredAgeWeeks(t, null), t.deliveredAgeWeeks ?? 0, t.id);
});
...
test('an in-service type is accepted, and arrives at the era-correct age', () => {
  ...
  // Classic: the market's delivered age — the published band, lifted by the
  // vintage ramp for a line closed 35+ years (the 727-200 closed in 1984).
  assert.equal(classic.fleet[0].ageWeeks, eraDeliveredAgeWeeks(getAircraftType('b727200'), null), 'classic: market delivered age');
  assert.ok(classic.fleet[0].ageWeeks > getAircraftType('b727200').deliveredAgeWeeks, 'classic: 42y closed is on the ramp');
});
