...
//
// Twin of Headwinds' tools/preview-honesty-test.mjs. Differences from that file
// are marked TW: — Tailwinds has no per-world demand multiplier, and its
// RouteDetail had two defects of its own that Headwinds does not share.
//
//   1. RouteDetail's pooled `combinedOffer` (the 2+ tails path) was scored at a
//      flat `qualityScore: <cond> ? 70 : 70` — a ternary whose branches are the
//      same literal — and carried neither the campaign lift, the loyalty/
//      reputation elasticity blunting, nor the lounge appeal that the tick
//      attaches. RoutePlanner's offer omitted the same two demand terms.
//
//   2. projectRouteAddition defaulted `eventDemandMult` to 1.0, so a world-event
//      demand shock reached a launch forecast only if the CALLER happened to
//      pass it. Both current call sites do; anything else previewed a world with
//      no pandemic in it.
//
//   3. projectWeek's lease-redelivery charge used the aircraft TYPE's list rate
//      while the reducer bills `a.weeklyLease ?? type.weeklyLease` — the rate the
//      tail actually signed at (term multipliers 1.15/1.00/0.90/0.83 stamp at
//      signing, so almost every lease differs).
//
...
//      frequency.
//
//   6. TW ONLY: RouteDetail's pooled demand allocation was WRITTEN under
//      `aircraft.id` and READ back under `route.id`, so on every multi-tail pair
//      the lookup missed and each tail fell through to a null demandOverride —
//      i.e. back to "what would this aircraft carry alone in this market?", the
//      exact double-count the pooling exists to prevent. The same fallback also
//      invented a business fare of `ecoP * 3.5` where the engine's own ladder
//      (CLASS_FARE_MULTIPLIERS.businessClass) is 2.5.
//
//   7. Marketplace's "weeks of cash" divided cash by lease rent alone, ignoring
...
const { AIRCRAFT_TYPES, getAircraftType } = await import('../src/data/aircraft.js');
const { getAirport } = await import('../src/data/airports.js');
const {
  weeklyTick, referencePrice, stateBrandReach,
  buildEventDemandModel, weekToGameDate, formatMoney,
  CLASS_FARE_MULTIPLIERS,
} = await import('../src/utils/simulation.js');
...
const { projectRouteAddition, pairMarketShare } =
  await import('../src/models/pairShare.js');
const { projectWeek } = await import('../src/utils/financeProjection.js');
const { currentGameDate: currentGameDateOf } = await import('../src/utils/simulation.js');
...
// capacity-capped, so demand-side effects (maturity, events) move the POOL
// without moving a single carried passenger — which makes it a useless probe for
// exactly the bugs below. Fly enough seats to outrun the pool.
const FREQ = 45;
...
// ─────────────────────────────────────────────────────────────────────────────
// TW: no worldDemandMult here — Tailwinds' tick multiplier for an O&D is the
// event model alone. What must hold is that the projection resolves it from
// state when no caller passes it, and does not double-apply it when one does.
console.log('\n── 2. Previews apply the world-event demand multiplier ─────────');

const PANDEMIC = { id: 'ev-test', title: 'Test shock', effects: { globalDemandMult: 0.6 } };

test('an ordinary world (no events) is unchanged', () => {
...
// by hand. Neither file may need editing, so the parameter keeps that meaning.
test('a caller passing the event multiplier by hand still agrees', () => {
  const { state, spare } = fixture({ activeEvents: [PANDEMIC] });
...
test('a caller passing the event mult on a SHARED pair still agrees', () => {
  const { state, spare } = fixture({ existing: 1, activeEvents: [PANDEMIC] });
  const ev = buildEventDemandModel(state.activeEvents).multFor(O, D);
  const r = previewGap(state, spare, { eventDemandMult: ev });
  assert.ok(r.gap <= 0.05,
    `previewed ${r.previewed} pax vs ${r.booked} booked (${(r.gap * 100).toFixed(1)}% off)`);
...
const baseSave = (over = {}) => ({
  ...
  competitors: [RIVAL], encroachments: {},
...
test('RoutePlanner\'s Est. Market Share is sensitive to the airline\'s brand', () => {
  ...
  assert.ok(strong > weak,
    ...
    'of this pair — the offer omits the demand terms the tick attaches');
});
...
test('the "Weekly Pax" total is one pooled market, not the pool once per tail', () => {
  ...
  assert.ok(shownPax <= engine * 1.10,
    ...
    'handed the whole pool because the allocation is keyed by aircraft and read by route');
});
