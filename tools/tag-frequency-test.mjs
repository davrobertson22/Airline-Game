...
//
//   node --import ./tools/_register-loader.mjs tools/tag-frequency-test.mjs
//
...
//     HEAD's cargo slot check                         = null   <- launches at 51/50
...
import {
  gameReducer, freshState, frequencyChangeBlockReason, addRouteBlockReason,
  peakSlotsUsedAt, slotsUsedAt,
} from '../src/store/GameContext.jsx';
import { SLOTS_PER_GATE } from '../src/utils/simulation.js';
...
test('interior stop out of slots blocks the increase', () => {
  ...
  assert.equal((s.gates.JFK ?? 0) * SLOTS_PER_GATE, SLOTS_PER_GATE);
...
test('ADD_CARGO_ROUTE will not launch onto slots a rotation occupies', () => {
  const s = tagState({ jfkGates: 1, jfkFlatFreq: 46, tagFreq: 2 });
...
test('two services cannot each take the full allowance on one exemption market', () => {
  ...
  s.gates.DFW = 9;
  const over = gameReducer(s, { type: 'ADD_TAG_ROUTE',
    aircraftId: 'a2', stops: ['DCA', 'DFW', 'LAX'], weeklyFrequency: 5 });
  assert.equal(over.routes.length, s.routes.length, '4 + 5 = 9 opened on a market capped at 7');
  // ...but 4 + 3 fits.
  const under = gameReducer(s, { type: 'ADD_TAG_ROUTE',
    aircraftId: 'a2', stops: ['DCA', 'DFW', 'LAX'], weeklyFrequency: 3 });
  assert.equal(under.routes.length, s.routes.length + 1,
    'a rotation that fits inside the shared allowance was refused');
});
