...
const eq  = (a, b, m) => (a === b ? ok() : bad(`${m} — got ${a}, expected ${b}`));
const truthy = (v, m) => (v ? ok() : bad(m));
...
// ── Allow-list hygiene: every entry present in this repo is two real airports
//    in the 2–65 km band (sub-35 km entries are spared from the same-metro backstop too) (a pair whose airport this build lacks is skipped) ──
