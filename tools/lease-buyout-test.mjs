...
// Tailwinds had no buyout at all — RENEW_LEASE and EXTEND_LEASE were the only
// ways to keep a leased tail, and neither ever made it yours. Headwinds already
// shipped one; porting it across is what turned up the bug below, which is why
// this suite lands with the feature rather than after it.
//
...
// purchase price set as a USED price, so discounting it again by total airframe
// age double-counts a discount already baked in. airframeNAV says so in its own
...
// profitable to launder this way, up to ~$42M a jet, in the live multiplayer
// game. It is not being ported. The deposit is a red herring — it cancels out of the round trip (paid
...
//
//   node --import ./tools/_register-loader.mjs tools/lease-buyout-test.mjs
...
import { airframeNAV, C_HOURS_DUE, D_HOURS_DUE } from '../src/data/maintenance.js';
import { leaseBuyoutQuote, leaseBuyoutPrice } from '../src/models/leaseBuyout.js';
...
test('the 8/4-week expiry toast offers all three ways out', () => {
  const src = readSrc('src/store/GameContext.jsx');
