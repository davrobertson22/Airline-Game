...
import { computeMarketShare, buildRouteMarket, computeUtility } from '../src/models/demand.js';
import { defaultAncillaries, routeAncillaries, ancillaryQualityBonus, ANCILLARY_MAP } from '../src/data/ancillaries.js';
import {
  WIFI_WEEKLY_OPEX, wifiInstallCost, wifiRetrofitCost, fleetWifiCoverage, groupWifiCoverage,
} from '../src/data/wifi.js';
import {
...
import { stateLoungeFields, routePairKey } from '../src/utils/simulation.js';
import { weeklyLoadJitter } from '../src/utils/market.js';
...
test('stateLoungeFields is the one answer the tick and every preview share', () => {
  ...
  // And the tick's own answer must equal the preview's. The tick also attaches
  // `loadJitter` (a deterministic per-pair, per-week wobble that previews
  // deliberately leave off — see the note in simulateRoute), so carry the same
  // figure here or the two disagree by the wobble and the guard reads as a
  // lounge bug when it is nothing of the sort.
  const jitter = weeklyLoadJitter(routePairKey(O, D), state.absWeek);
  const withJitter = simulateRoute(
    { ...state.routes[0], classPrices: state.routePricing[PAIR], ...fields, loadJitter: jitter },
    state.fleet[0], { week: 1, month: 6 }, null, 1, null, [], null, null, 1, state.ancillaries);
  const rr = weeklyTick(state).routeResults.find(r => r.routeId === 'r1');
  assert.equal(rr.loungeCost, withJitter.loungeCost,
