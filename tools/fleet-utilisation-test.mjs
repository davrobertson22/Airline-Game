...
const { gameReducer } = await import('../src/store/GameContext.jsx');
...
const { scheduleTrimNews } = await import('../src/models/newsLog.js');
...
test('the trim produces a news entry naming the aircraft, route and frequencies', () => {
  ...
  // One durable news row per AIRCRAFT — never one per frequency decrement — and
  // it must land in the save's own log, not a toast.
  assert.equal(typeof scheduleTrimNews, 'function',
    'there is no builder for the schedule-trim news row');
  const rows = scheduleTrimNews([notice], { absWeek: 200, year: 4, week: WEEK });
  assert.equal(rows.length, 1, 'one row per aircraft, not one per frequency decrement');
  assert.equal(rows[0].kind, 'schedule_trim');
  const logged = (after.newsLog ?? []).filter(n => n.kind === 'schedule_trim');
  assert.equal(logged.length, 1, 'the trim did not reach the save\u2019s news log');

  const d = rows[0].data;
  const text = `${rows[0].subject} ${d.aircraft} ${Math.round(d.peakBefore)}h ${d.capHours}h `
             + d.cuts.map(c => `${c.origin}–${c.destination} ${c.fromFrequency} → ${c.toFrequency}`).join(' ');
  assert.match(text, /N1RSV/, 'the entry does not name the aircraft');
  assert.match(text, new RegExp(`${MAX_WEEKLY_BLOCK_HOURS}h`), 'the entry does not state the limit');
  assert.match(text, /JFK–BOS/, 'the entry does not name the route');
  assert.match(text, new RegExp(`40\\s*→\\s*${cut.toFrequency}`), 'the entry does not give both frequencies');
});
