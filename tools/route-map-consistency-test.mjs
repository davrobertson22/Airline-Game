...
// three pairs already carry an AI encroacher big enough to take a real bite of
// the pool, so the engine's load factor lands well under 100% — which is exactly
// the number the old map could not see.
...
const FREQ = 32;   // enough capacity that a contested pool cannot fill it
const HUB = 'SFO';
...
// Encroachers on the first TWO pairs only — the third stays a monopoly, so the
// suite also proves the fix doesn't move uncontested routes.
const encroachments = {};
for (const dest of SPOKES.slice(0, 2)) {
  encroachments[routePairKey(HUB, dest)] = {
...
const save = {
  ...
  // The fixture frequency is deliberately above the 140h weekly block-hour cap
  // — it wants capacity a contested pool cannot fill. Marking the save as
  // already through the one-off over-cap trim keeps loading it from rewriting
  // the frequencies under the suite; the subject here is the MAP agreeing with
  // the engine, not the migration.
