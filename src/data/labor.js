...
export function crewScaleFor(groupId, aircraftType) {
  ...
  // Concorde's 128 seats say nothing about what it takes to crew. No seat count
  // falls back to the category, never to the smallest anchor.
...
export function splitStarterHire(groupId, count, headcount, fleet, typeOf) {
  ...
  // which is exactly the wait this perk exists to waive. Math.max pins negative
  // zero to 0: Math.ceil(-1e-9) is -0, and assert.strict treats those as unequal.
...
export function laborEffects(labor, avgUtilization = null, satisfaction = null) {
  ...
  // classic save, every preview caller) → zero, so nothing moves.
