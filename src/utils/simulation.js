...
import { cargoCityPairDemand, cargoReferenceYield, referencePrice,
         ...
         loadDemandScale, weeklyLoadJitter, LOAD_CEILING,
         metroPairKeyOf, memberPairKeysOf } from './market.js';
...
// The price used to live in src/components/FleetConfig.jsx alone, so the
// reducer could not re-derive it (it charged whatever the modal handed it) and
// could not know how long the work takes. The modal and the reducer read the
// same two functions now.
//
// NOTE: Tailwinds prices refits on its own terms — a flat fee per quality tier
// stepped on either axis, seat AND service. Headwinds dropped service quality
// and moved to a per-premium-seat install fee, so the two games deliberately
// disagree here. Do not "sync" one onto the other without a balance decision.
export const REFIT_SEAT_COST      = 2_500;   // per seat moved between classes
export const REFIT_QUALITY_STEP   = 30_000;  // per quality tier changed, either axis
export const REFIT_MIN_COST       = 10_000;  // any refit at all costs at least this
export const REFIT_QUALITY_LEVELS = { basic: 0, standard: 1, premium: 2, luxury: 3 };
...
function qualitySteps(current, next) {
  const lvl = (v) => REFIT_QUALITY_LEVELS[v ?? 'standard'] ?? 1;
  return Math.abs(lvl(next?.seatQuality)    - lvl(current?.seatQuality))
       + Math.abs(lvl(next?.serviceQuality) - lvl(current?.serviceQuality));
}

/**
 * One-time cost to reconfigure a cabin: per seat moved between classes, plus a
 * flat fee per quality tier stepped on either axis. Returns 0 for a no-op.
 */
export function calcReconfCost(current, next) {
  const seatChanges = refitSeatsMoved(current, next);
  const steps       = qualitySteps(current, next);
  if (seatChanges === 0 && steps === 0) return 0;
  return Math.max(REFIT_MIN_COST, seatChanges * REFIT_SEAT_COST + steps * REFIT_QUALITY_STEP);
}
 ...
 *
 * A preview that computes this itself is a bug waiting to be reported. The Add
 * Flights form's utilisation bar summed `routes.filter(r => r.aircraftId === id)`
 * at the DIRECT O&D distance, so a tag rotation cost it only its end-to-end hop
 * and a rotation out on cover cost it nothing at all: the bar read 136 / 140h
 * and green, the picker on the same screen read 11h free, and the submit came
 * back "no spare flying hours — this would need 151h/wk". Call this instead.
 *
 ...
 *
 * Called from reconcileState — every save load. (Headwinds runs the same pure
 * function on the authoritative server blob as well; see its tickService.)
 */
...
export function simulateRoute(route, aircraft, gameDate = { month: 6 }, labor = null, fuelMultiplier = 1.0, demandOverride = null, encroachmentSpecs = [], avgUtilization = null, satisfaction = null, eventDemandMult = 1.0, ancillaries = null, competitors = null, rivalIndex = undefined) {
  ...
  //
  // loadDemandScale divides by the demand it is given, so its result must be
  ...
  const loadScaleRaw = loadDemandScale(uncappedOneWay, totalCapOneWay, route.loadJitter ?? 1);
  const loadScale = (loadScaleRaw !== 1 && pool0 > 0)
    ? Math.min(1, (uncappedOneWay * loadScaleRaw) / pool0)
    : 1;
  if (loadScale !== 1) {
 ...
 *
 * Exported because weeklyTick and any client-side share preview must produce the
 * SAME number — when this lived only as a closure inside weeklyTick, previews
 * silently passed 0 and under-reported the player's share.
 */
 ...
 * weeklyTick's `brandReachFor`. Any screen that builds a player offer itself
 * must include this field; omitting it would show a brand-new airline the
 * market share of an established one.
 *
...
// fresh array every tick (tickCompetitorAI maps to clones), so remembering the
// derivation per ARRAY is exact. Projection-only, like the readers above.
...
export function weeklyTick(state) {
  ...
  // Delegates to the exported helper so client-side share previews compute the
  // identical figure instead of quietly omitting it.
  const sensReductionFor = (hubQ) =>
    priceSensitivityReductionFor(repElasticityRed, loyaltyStrength, loyaltyTierNow, hubQ);

  // Brand reach: awareness × reputation × loyalty × alliance, net of rival ad
  // pressure — ONE number describing how much of a market will even consider
  // flying this airline.
  //
  // It used to be `combinedMult`, a multiplier applied to route REVENUE after
  // the share fight and after the capacity cap. That was wrong three ways:
  //
...

  {
    ...
    // (measured: 2,574 pax against 1,292 for the same metal on one pair), and a
    // rival on a sibling airport was invisible (1,292 contested vs 1,292 alone).
    ...
    for (const [, pairKeys] of lanes) {
      ...
      // byte — simulateRoute runs exactly the fight it always did. (This is the
      // old `group.length < 2` + "one tag route's own segments" pair of guards,
      // restated lane-wide: a tag route that is the lane's only presence still
      // self-contains its split inside simulateTagRoute.)
      ...
      for (const pairKey of pairKeys) {
        ...
        // The type premium is applied group-wide afterwards, exactly as before.
        ...
        const baseEcoPrice = ecoPriceSeats > 0
          ? ecoPriceWeighted / ecoPriceSeats
          : (cp0.economy ?? r0.ticketPrice ?? 1);
        const ecoPrice = Math.max(1, baseEcoPrice * ticketPremium);
