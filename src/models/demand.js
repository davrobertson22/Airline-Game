 ...
 * secondary field scored at parity. Measured before this: a lone SWF–STN route
 * booked 1,270 pax against 1,300 for the same metal on JFK–LHR — ratio 0.977,
 * i.e. the whole New York↔London pool out of Newburgh.
 *
...
export function computeMarketShare(market, offers) {
  ...
  return offers.map((offer, i) => {
    ...
    // consulted. The load models downstream (loadDemandScale,
...
export function intlDestinationsFrom(routes, code) {
  ...
  for (const r of routes ?? []) {
    ...
    for (const s of stops) {
      ...
      // "International" against the SOVEREIGN state (data/territories.js), so
