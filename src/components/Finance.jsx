...
const CLASS_COLORS = { firstClass: '#bc8cff', businessClass: '#d29922', premiumEconomy: '#388bfd', economy: '#3fb950' };
        ...
        <table>
          ...
          <tbody>
            {airports.map(({ code, airport, routes: aptRoutes, revenue, pax }) => {
              ...
              const tierColor  = { mega: '#a371f7', major: '#388bfd', regional: '#3fb950' }[airport?.tier] ?? 'var(--text-muted)';
...
const GRADE_CONFIG = [
  { min: 0.020, label: 'A', color: '#3fb950' },
  { min: 0.010, label: 'B', color: '#4fc3f7' },
  { min: 0.000, label: 'C', color: '#d29922' },
  { min: -Infinity, label: 'D', color: '#f85149' },
];
...
function creditRating(state, weeklyRevenue, weeklyNetIncome) {
  ...
  if      (score >= 85) return { grade: 'A', label: 'Excellent',  color: '#3fb950', rateBonus: -0.02 };
  else if (score >= 70) return { grade: 'B', label: 'Good',       color: '#4fc3f7', rateBonus: -0.01 };
  else if (score >= 55) return { grade: 'C', label: 'Fair',       color: '#d29922', rateBonus:  0.00 };
  else if (score >= 40) return { grade: 'D', label: 'Poor',       color: '#f0883e', rateBonus:  0.02 };
  else                  return { grade: 'F', label: 'High Risk',  color: '#f85149', rateBonus:  0.05 };
}
