...
import { rivalIndexFor, rivalOneStopOffersFor, rivalsOn, RIVAL_CONN_PREFIX } from '../models/network.js';
 ...
 *  winter schedule. Every player-vs-competitor comparison has to fold those
 *  together — otherwise whichever row happened to be written last was the only
 *  one shown and your own frequency/seats were silently undercounted.
 *
    ...
    <div className="card" style={{ padding: '12px 14px' }}>
      ...
      <div style={{
        ...
        {competitors.map(c => {
          ...
          const seats = cfg.aircraftType ? getAircraftType(cfg.aircraftType)?.seats ?? null : null;
          return (
            <div key={c.id} style={{ padding: '7px 8px', textAlign: 'center', color: 'var(--text-muted)' }}>
              {seats != null ? (seats * (cfg.frequency ?? 0)).toLocaleString() : '–'}
            </div>
          );
        })}
        {viaRoutings.map(v => (
    ...
    <div style={{
      ...
      <div className="card" style={{ width: '100%', maxWidth: 420, padding: '24px 24px 20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 13, marginBottom: 16 }}>
          ...
          <DealRow label="You receive · staff" value="Their crews" color="var(--green)" />
        ...
        {routeCount > 0 && (
          <div style={{ background: 'var(--surface2)', borderRadius: 8, padding: '10px 12px', marginBottom: 16, fontSize: 12 }}>
            <div style={{ fontWeight: 600, marginBottom: 6, color: 'var(--text-muted)' }}>Routes to absorb (fleet, crews & gates come with them)</div>
