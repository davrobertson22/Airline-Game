...
const WIKI = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { h: 'Maintenance' },
      { p: 'Maintenance has its own page now, under Company. Pay rates for the maintenance team still live here, because they gate automatic check scheduling.' },
  ...
  },
  {
    id: 'maintenance',
    icon: '\uD83D\uDD27',
    title: 'Maintenance & Jet Bases',
    blurb: 'Keeping the fleet airworthy',
    blocks: [
      { p: 'Maintenance is three costs pulling in different directions: the routine upkeep you choose to fund, the heavy checks the calendar forces on you, and the breakdowns you cannot predict. The Maintenance page under Company shows all three in one place.' },
      { h: 'Line maintenance' },
      { p: 'The **maintenance budget** covers day-to-day upkeep, parts and components. Higher spending keeps aircraft reliable and slows ageing; cutting it saves money now but raises breakdown risk and wears airframes out faster.' },
      { h: 'Heavy checks' },
      { p: 'Every airframe is due a **C check** roughly every 4,500 block hours or two years, and a **D check** every 24,000 hours or six years. A D check also resets the C clock and knocks years off the airframe\u2019s effective age. Miss the grace window and the regulator grounds the aircraft itself: longer downtime, 50% more cost, and a reputation hit. Pay the maintenance team at least 1.30\u00d7 market **and** set the budget to 1.30\u00d7 and due checks book themselves.' },
      { h: 'Breakdowns cost money' },
      { p: 'An unscheduled mechanical failure carries a **repair bill**, not just downtime, and the bill climbs steeply with the airframe\u2019s age: an old jet costs several times what a new one does to fix. If a repair would cost more than the aircraft is worth, it is **written off** instead \u2014 hull insurance pays out on owned metal, and the aircraft leaves your fleet.' },
      { h: 'Jet bases' },
      { p: 'You can build your own maintenance infrastructure. A **jet base** sits at one airport and is certified for specific aircraft **families**. Three levels: a **Line Station** handles breakdowns, a **Maintenance Base** brings C checks in-house, and a **Heavy MRO** does D checks too.' },
      { p: 'A base helps an aircraft only when all three of these are true: it is certified for that aircraft\u2019s family, it sits on an airport that aircraft\u2019s routes touch, and it still has a **shop slot** free that week. Everything that overflows goes to a third party at full price and full downtime.' },
      { p: 'Bases need **gates** at their airport, and the hangar occupies them \u2014 those gates stop being available for flying. At a congested airport that is a real trade.' },
      { warn: 'A new base opens at 60% effectiveness and takes about six months to reach full strength. Building one the week your fleet starts falling apart is too late.' },
      { h: 'Outsourced contracts' },
      { p: 'Every aircraft family you operate carries a fixed weekly **MRO contract**, whatever the fleet size \u2014 which is why flying nine families is so much more expensive than flying two. A certified base does that work in-house and offsets most of the family\u2019s contract, up to 85% at a Heavy MRO. Concentrate a family through one base and it goes a long way toward paying for itself.' },
      { h: 'Parts pool' },
      { p: 'Each base holds a **spares inventory** you can dial from 0.5\u00d7 to 2.0\u00d7. A deeper pool ties up cash every week but gets grounded aircraft flying again sooner.' },
    ],
  },
  {
    id: 'reputation',
  ...
  },
  {
    ...
    blocks: [
      ...
      { h: 'Rival connections' },
      { p: 'Rivals hub too. A carrier with a hub at Frankfurt and flights to New York and Amsterdam sells **New York–Amsterdam via Frankfurt** — one itinerary, two of its flights — and it competes on that pair even though it never flies it nonstop. Rival connections go through the same demand model as everything else: a connection is penalised for the stop, priced as the sum of its two legs, and limited to the seats the rival protects for connecting passengers, so you beat it on fare, frequency and quality. Routings that backtrack (more than 1.5× the nonstop distance) are never sold. Your own connections over your hubs face rival connections the same way. The contested-route card shows each rival routing as its own **via HUB** column.' },
      { h: 'Head-to-head' },
