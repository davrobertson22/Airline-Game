  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: H, display: 'block' }}>
        ...
        {/* Profit area fill */}
        <path d={profitArea} fill="#3fb950" opacity="0.07" />

        {/* Lines */}
        {line(revenues, '#388bfd')}
        {line(costs,    '#f85149', '5 3')}
        {line(profits,  '#3fb950')}

        {/* End dots */}
        <circle cx={px(n-1).toFixed(1)} cy={py(revenues[n-1]).toFixed(1)} r="3.5" fill="#388bfd" />
        <circle cx={px(n-1).toFixed(1)} cy={py(costs[n-1]).toFixed(1)}    r="3.5" fill="#f85149" />
        <circle cx={px(n-1).toFixed(1)} cy={py(profits[n-1]).toFixed(1)}  r="3.5" fill="#3fb950" />
      </svg>
      <div style={{ display: 'flex', gap: 16, fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
        <span><span style={{ color: '#388bfd' }}>—</span> Revenue</span>
        <span><span style={{ color: '#f85149' }}>- -</span> Costs</span>
        <span><span style={{ color: '#3fb950' }}>—</span> Profit</span>
